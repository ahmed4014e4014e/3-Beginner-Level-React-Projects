...
export const PersonStyle = styled.li`
  ...
  font-weight: 500;
`;
export default function PersonalList({personal, searchName, handlePersonClick}) {
  return (
    <section>
      ...
      <PersonalCanvas className='mb-1'>
        <ul>
          {personal.map((person) => (
            <PersonStyle className='bg-dark text-light' key={person.id} onClick={()=>handlePersonClick(person)}>{person.first_name} {person.last_name}</PersonStyle>
          ))}
        </ul>
      </PersonalCanvas>
