...
export default function PersonalDetails({ selectedPerson }) {
    const { first_name, last_name, date_of_birth, yearly_leave_days} = selectedPerson;
    return (
        <section>
            <h2 className="alert alert-accept">{first_name} {last_name}</h2>
            <div className="card">
                <h2 className="fs-md">Personal details</h2>
                <div className="card-text">
                    <p>Date of birth: {date_of_birth}</p>
                    <p>Age: {calculateAge(date_of_birth)}</p>
        	        <p>Yearly leave days: {yearly_leave_days} </p>
