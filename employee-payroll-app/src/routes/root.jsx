...
export default function Root() {
  ...
  return (
    <div className='App container mt-4 p-1 d-flex shadow'>
      <nav className='navbar bg-accept text-light mb-3' style={{ height: "750px" }}>
        <ul className='navbar-nav me-auto' >
          ...
          {authenticator.isAuthenticated && <NavLink to={`/personal/`} className={({ isActive }) => isActive ? "nav-item active" : "nav-item"}>Personal</NavLink>}
          {authenticator.isAuthenticated && <NavLink className={"text-warning"} onClick={authenticator.logout}>Logout</NavLink>}
        </ul>
      </nav>
      <Outlet />
    </div>
